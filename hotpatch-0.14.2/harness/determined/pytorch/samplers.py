...
.. note::
   ...
   If you are using a MapDataset and not an IterableDataset, make your life easy and use the
   det.pytorch.DataLoader() which is a drop-in replacement for pytorch's built-in DataLoader, and
   Determined will handle all of these details.
...
- Even if you are going to ultimately return an IterableDataset, it is best to use PyTorch's Sampler
  ...
  vs sequential access at TODO:ref_yogadl_readme.  If you don't have a custom sampler, start with a
  simple one:
...
- **Skip when training, and always last**: In Determined, training datasets should always be able to
  ...
  Always skip AFTER your repeat, so that the skip only happens once, and not on every dataset.
...
class DistributedSampler(torch.utils.data.Sampler):
    ...
    def __len__(self) -> int:
        all_workers_get_samples = len(self._sampler) // self._num_workers
        worker_gets_extra_sample = int(len(self._sampler) % self._num_workers > self._rank)
...
class ReproducibleShuffleBatchSampler(torch.utils.data.Sampler):
    ...
    def __init__(self, batch_sampler: torch.utils.data.Sampler, seed: int) -> None:
